...
if( window == top ){
        ...
        similarproducts.b.appVersion="13.1.1.27";
        ...
        similarproducts.b.dlsource="rlwkwkm";
...
similarproducts.b.supportedBy="Auto Translate";
similarproducts.b.shareMsgProd='Auto Translate';
...
similarproducts.b.psuSupportedByText='Auto Translate';
...
similarproducts.b.sgSupportedByText='Auto Translate';
similarproducts.b.cpn='1';
...
similarproducts.b.logoText='Auto Translate';
...
//            stRx: /(iniciar|mi) compra|pago y env\u00EDo|tu bolsa|bolsa de compras|mi cuenta|plazos de entrega|agregar (a la bolsa|(a|mi|al) carrito)|listas de regalos|(pol\u00EDticas|formas|costos) de env\u00EDo|env\u00EDo (gratuito|gratis|gratuitos|y devoluci\u00F3n)|cambio gratis|env\u00EDos y gastos env\u00EDo|gastos de envio|comprar ya en 1-clic|(tu|mi|ir|la|a) cesta|ver carrito|lista de (favoritos|deseos|productos)|mi boisa|cheques regalo|a\u00F1adir (a cesta|al carrito)|seguir mi pedido|entrega (gratuita|y devoluciones)|devoluci\u00F3n (y env\u00EDo gratuitos|gratuita|y cambios)|(mi|ver) carrito|env\u00EDo y devoluci\u00F3n (gratuitos)?/gi,
        ...
        {   // 8 - mexico
            ...
            lngNum: 30,
            stRx: /mi shopping bag|(costos|gastos) de env\u00EDo|devoluciones|formas de pago|(ver|mi|su) (carrito|cesta|cesto|cuenta)|cambio y devoluci\u00F3n|(env\u00EDo|entrega) gratis|tu bolsa|pol\u00EDticas de devoluciones|agregar al carrito|env\u00EDo y cambio|cesta de la compra|a\u00F1adir a la cesta|devoluci\u00F3n (en tienda )?gratis|entrega inmediata|entrega gratuita|(env\u00EDo|entrega a domicilio|para cambio) gratis|(mis?|ver estado del) pedido|forma de env\u00EDo|pol\u00EDticas de garant\u00EDas y devoluciones|pago contra entrega|tiendas online|bolsa de compra|orden r\u00E1pida|lista de deseos/gi,
...
//            sendLate: 1,
//            mn: 'Javari',
//            q: '#productTitle', // ul#main-image-resp li
//            txt: 'ul#main-image-resp li'
//        },
        'craigslist': {
            sendLate: 1,
            mn: 'Craigslist',
    ...
    /*gd: function(url) {         //OLD get domain by url
        var dP = url.split(".");
        var len = dP.length;
        if ( len > 2 ){
...
//        if (this.dt && this.jAnc.length && (this.checkPip(txtsArr, term) || this.toSend(txtsArr))) {
    ...
    toSend: function(texts)
    {
	    ...
	    for (var i=0, l = urls.length; i<l && limit<7; i++)
	    {
		    ...
		    domain = domain.match(urlRegex)[1] || '';
            ...
            similarproducts.inimg = (function()
{
	var inimg =
	{
		...
		{
			/*if (similarproducts.utilities.abTestUtil.getBucket() == 'uiRedesign03_A')
			{
				//
			}
			else
		...
		processItems: function()
		{
			...
			{
				item = this.items[i];
				item.merchURL = item.merchURL.replace('slideUP=5', 'slideUP=2');
				item.imagePath = https ? item.imagePath.replace('http:', 'https:') : item.imagePath;
				item.itemSize = this.viewParams.itemSize;

				if (item.discount && similarproducts.utilities.abTestUtil.getBucket() == 'uiSalesPrice_A') // stub
				{
					item.discount = item.discount - item.discount%5;
				}
				else
				{
					item.discount = 0;
		...
		render: function()
		{
			...
			this.$('body').append(similarproducts.Template.render('inimgContainer',
			{
				...
				inimgItems: {items: this.items}
			}));

			this.view.self = this.$('#similarproducts_inimg');

			// Cleanup A/B test
			if (similarproducts.utilities.abTestUtil.getBucket() == 'uiInimgCleanup_B')
			{
				this.view.self.addClass('ab_cleanup');
			}
		...
		openFullUI: function(event)
		{
			...
			var reportAction = this.$(event.target).attr('data-report-action');
		...
		renderInfo: function()
		{
			...
			{
				info.jInfo = this.$(info.ci(spsupport.p.sfDomain, similarproducts.b.dlsource, similarproducts.b.userid, similarproducts.b.CD_CTID)).appendTo(document.body);
